...
model{ //Begin block
  ...
  for(h in 1:3){                   //For population in 1 - 3
    for(i in 1:2){                 //For sex in 1 - 2
      for(j in 1:2){               //For phase in 1 - 2
        a[h,i,j] ~ normal(3, 1);   //For fixed effects, assign non-'explosive' prior - see section 11.2 in Rethinking: https://github.com/Booleans/statistical-rethinking/blob/master/Statistical%20Rethinking%202nd%20Edition.pdf
      }                            //End j
    }                              //End i
  }                                //End h
  ...
  for(i in 1:98){                                                          //For every bird
    if(skip[i] == 0){                                                      //If bird not designated a 'skip'
      switches[i] ~ poisson(exp(a[pop[i], sex[i], phase[i]] + b[id[i]] )); //Estimate influence of pop, sex & phase on total-switches-in-test
    }                                                                      //End i
  }                                                                        //End j
} //End block                                        
