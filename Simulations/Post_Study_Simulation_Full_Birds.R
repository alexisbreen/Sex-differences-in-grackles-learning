...
#Code for performing the post-study model validation simulations (Figure S3) for the manuscript 
...
Post_Study_Sim_Fxn <- function(N_block = 30,  #Maximum number of blocks (10 trials per block)
                               N_trials = 10, #Trials per block   
                               N_sim = 1)     #From just one sim get 32 males & 17 females i.e., matched to full data set
...
{ #Define function operations
  ...
  for(sim in 1:N_sim){ #For every sim
    ...
    Female <- rmvnorm(17, c(lambda_means[draw,1,1], lambda_means[draw,1,2], phi_means[draw, 1, 1], phi_means[draw, 1, 2]), S[draw,,])
    Male   <- rmvnorm(32, c(lambda_means[draw,2,1], lambda_means[draw,2,2], phi_means[draw, 2, 1], phi_means[draw, 2, 2]), S[draw,,])
    ...
    for(ind in 1:49) { #For every bird 
      ...
      d <- data.frame(id = ifelse(sim == 1, ind, ((49 * sim) - 49) + ind), #Generalises to more than 1 sim
                      sex = ifelse(ind <= 17, 1, 2),  
...
dat <- Post_Study_Sim_Fxn() #Name and execute our data simulation, here you can also assign other parameters values 
