...
//Code to run STAN Poisson regression modelling total-trials-in-test between populations for the manuscript
...
data{ //Begin block
  ...
  int pop[98];    //Population identification i.e., core (1), middle (2) or edge(3)
...
parameters{ //Begin block
  ...
  real a[3,2,2]; //Fixed effects - indexed by pop, sex, phase
...
model{ //Begin block
  ...
  for(h in 1:3){                   //For population in 1 - 3
    for(i in 1:2){                 //For sex in 1 - 2
      for(j in 1:2){               //For phase in 1 - 2
        a[h,i,j] ~ normal(4, 0.5); //For fixed effects, assign non-'explosive' prior - see section 11.2 in Rethinking: https://github.com/Booleans/statistical-rethinking/blob/master/Statistical%20Rethinking%202nd%20Edition.pdf
      }                            //End j
    }                              //End i
  }                                //End h
  ...
  for(i in 1:98){                                                        //For every bird
    if(skip[i] == 0){                                                    //If bird not designated a 'skip'
      trials[i] ~ poisson(exp(a[pop[i], sex[i], phase[i]] + b[id[i]] )); //Estimate influence of pop, sex & phase on total-trials-in-test
    }                                                                    //End i
  }                                                                      //End j
} //End block
